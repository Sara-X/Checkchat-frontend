...
function Messagecolor(props){
    var doms=[]
    for(var i=0;i<props.data.data.length;i++){
        if(i%2===0){
            doms.push(<div style={{display:"inline-block"}}>{props.data.data[i] }</div>)
        }
        if(i%2===1){
            doms.push(<div style={{display:"inline-block"}} className="msg_color">{props.data.data[i] }</div>)
        }
    }
    return doms
}
        ...
        <Transition key={props.key} transitionOnMount={true} animation='scale' duration={400}>
            <div className='robot-message'>
                ...
                <div className='msg_cont' onTouchStart={handleContentPress} onTouchEnd={handleContentRelease}>
                     <Messagecolor data={props.props} flag={0} style={{display:"inline-block"}}/>
                </div>
	...
	return (
		<Transition key={props.key} transitionOnMount={true} animation='scale' duration={400}>
			<div className='user-message'>
				<div ref={ref}  className='msg_icon'>
					<Image className='circle clickable' src={user} size='mini' onClick={() => props.setToggle(!props.toggle)} />
				</div>
				<div className='msg_cont' onTouchStart={handleContentPress}  onTouchEnd={handleContentRelease}>
